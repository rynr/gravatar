...
public class GravatarTest {
    ...
    @Test
    public void verifyDefaultUrlCanBeSet()
            throws UnsupportedEncodingException, GravatarException {
        assertThat(
                Gravatar.forEmail(EXAMPLE_EMAIL)
                        .defaultImage(EXAMPLE_DEFAULT_URL).toUrl(),
                equalTo(GRAVATAR_URL_FOR_EXAMPLE_EMAIL + "?d=" + URLEncoder
                        .encode(EXAMPLE_DEFAULT_URL, GRAVATAR_CHARSET)));
    }
    ...
    public void verifyDefaultUrlCanBeCleared()
        ...
        assertThat(builder.toUrl(), equalTo(GRAVATAR_URL_FOR_EXAMPLE_EMAIL
                + "?d="
                + URLEncoder.encode(EXAMPLE_DEFAULT_URL, GRAVATAR_CHARSET)));
        assertThat(builder.defaultImage((String) null).toUrl(),
                equalTo(GRAVATAR_URL_FOR_EXAMPLE_EMAIL));
    }

    @Test
    public void verifyCompleteExample() throws MalformedURLException,
        ...
        for (String pair : pairs) {
            ...
            final String key = idx > 0 ? URLDecoder
                    .decode(pair.substring(0, idx), GRAVATAR_CHARSET) : pair;
            final String value = idx > 0 && pair.length() > idx + 1 ? URLDecoder
                    .decode(pair.substring(idx + 1), GRAVATAR_CHARSET) : null;
